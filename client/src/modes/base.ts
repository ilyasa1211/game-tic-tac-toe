...
import { GameResultEventDraw, GameResultEventWin } from "../events/game-result.ts";
...
export abstract class OfflineMode implements IGameMode {
	...
	// cached status
	private gameOverStatus: typeof GameOverStatus[keyof typeof GameOverStatus] | undefined;
	...
	public async getGameOverStatus(): Promise<typeof GameOverStatus[keyof typeof GameOverStatus]> {
		// cache
		...
		const calculateResult = async () => {
			...
			let winPositionOffset = system.WIN_POSITION_OFFSET;
			...
			for (let row = 0; row < size - 2; row++) {
				for (let col = 0; col < size - 2; col++) {
					const position = [
						positions[row * size + col], positions[row * size + col + 1], positions[row * size + col + 2],
						positions[(row + 1) * size + col], positions[(row + 1) * size + col + 1], positions[(row + 1) * size + col + 2],
						positions[(row + 2) * size + col], positions[(row + 2) * size + col + 1], positions[(row + 2) * size + col + 2],
					];

					console.log(`3x3 at row=${row}, col=${col}:`, position);
					...
					for (let i = 0; i < system.WIN_POSITIONS.length; i++) {
						...
						if ([blockOne, blockTwo, blockThree].some(block => typeof block === "undefined")) {
							continue;
						}
